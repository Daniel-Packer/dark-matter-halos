...
def kmeans_downsample_points(
    ...
    padded_clouds = jnp.stack(
        [
            jnp.concatenate(
                [cloud, jnp.zeros([max_cloud_size - cloud.shape[0], *cloud.shape[1:]])]
            )
            for cloud in points_list
        ],
        axis=0,
    )
    padded_weights = jnp.stack(
        [
            jnp.concatenate(
                [jnp.ones([cloud_size]), jnp.zeros([max_cloud_size - cloud_size])]
            )
        ...
        if require_convergence:
            try:
                ...
                raise AssertionError(
                    f"Convergence failed :( in {100 * jnp.mean(1 - group_output.converged):.1f}% of cases"
                )
    else:
        ...
        for i in range(n // outer_loop_size + 1):
            ...
            if require_convergence:
                try:
                    ...
                    raise AssertionError(
                        f"Convergence failed :( in {100 * jnp.mean(1 - group_output.converged):.2f}% of cases"
                    )
    ...
    def get_label_freqs(valid_assignment):
        labels, freqs = jnp.unique(
            valid_assignment, return_counts=True, size=downsample_size + 1
        )
